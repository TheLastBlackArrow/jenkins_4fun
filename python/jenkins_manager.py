...
def collect_agents_and_generate_casc(agent_service_name: str) -> None:
    ...
    agent_containers = [
        ...
        if c.labels.get("com.docker.compose.service") == agent_service_name
    ]

    if not agent_containers:
        logging.error("No containers found for Docker Compose service %s.", agent_service_name)
        sys.exit(1)

    for container in sorted(agent_containers, key=lambda c: c.name):
        for _ in range(20):
            container.reload()
            ...
            time.sleep(2)
        if container.status != "running":
            logging.error("Agent container %s did not reach running state.", container.name)
            sys.exit(1)
        ...
        if networks:
            for net_data in networks.values():
                ...
                if ip:
                    break
        if not ip:
            logging.error("No IP address found for agent container %s.", container.name)
            sys.exit(1)
        agent_ips.append(ip)
